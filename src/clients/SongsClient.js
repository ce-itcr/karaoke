...
export class SongsClient {
    async getAllSongs() {
        const url = "https://karaoke-api-develop.herokuapp.com/api/v1/songs/all"
        const response =  await axios(url)
        return response;
    }

    async getSongById(songId){
        const url = "https://karaoke-api-develop.herokuapp.com/api/v1/songs/" + songId
        const response =  await axios(url)
        return response;
    }

    async searchSongs(category, filter) {
        const url = 'https://karaoke-api-develop.herokuapp.com/api/v1/songs/search/{"category":"' + category + '","filter":"' + filter + '"}';
        const response =  await axios(url);
    ...
    async createSong(songName, songAuthor, songAlbum, creationAuthor, songMP3, songLRC, songCover){
        const requestUrl = 'http://karaoke-api-develop.herokuapp.com/api/v1/songs/';
    ...
    async updateSong(songId, songMP3, songCover, songLRC, modificationAuthor) {
        const requestUrl = 'https://karaoke-api-develop.herokuapp.com/api/v1/songs/' + songId;
