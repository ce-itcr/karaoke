...
export default function GreaterDifficulty() {
    ...
    useEffect(() => {
        getGreaterDifficultyWords();
    },[])

    const getGreaterDifficultyWords = async() => {
        const response = await statsClient.findUserWords(localStorage.getItem('currentUsername'));
        setWords(response.data.mostDifficult)
    }
                    ...
                    {words.map((item,index)=>{
                        return  <tr key={index}>            
                                ...
                                <div className="flex items-center">
                                    ...
                                    <div className="overflow-hidden h-2 text-xs flex rounded bg-red-500">
                                        <div
                                        style={{ width: item[3] }}
